...
export async function uploadToApiVideo(
  ...
  if (!apiKey) {
    console.warn(
      "[api.video] API key not configured, falling back to Cloudinary",
    );
    return null;
  }
  ...
  try {
    ...
    if (file.size < 200 * 1024 * 1024) {
      ...
      return {
        ...
        thumbnailUrl:
          uploadedVideo.assets?.thumbnail ||
          `https://vod.api.video/vod/${videoId}/thumbnail.jpg`,
      ...
      console.warn(
        "[api.video] Large file detected, chunked upload not yet implemented",
      );
      return null;
    }
  } catch (error) {
